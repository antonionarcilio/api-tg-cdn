...
describe('filterMediaItems', () => {
  ...
  it('matches description case/accent-insensitively', () => {
    ...
    expect(filterMediaItems(videos, { description: '#jeangrey' })).toHaveLength(1);
    expect(filterMediaItems(videos, { description: 'jeangrey' })).toHaveLength(1);
  ...
  it('requires all provided filters to match (AND semantics)', () => {
    ...
    expect(filterMediaItems(videos, { chatId: '1', fileName: 'a', description: '#jeangrey' })).toHaveLength(1);
  });
...
describe('filterByMediaText', () => {
  ...
  it('filters by description', () => {
    expect(filterByMediaText(items, { description: '#jeangrey' })).toEqual([items[0]]);
  });

  it('requires file_name and description to match when both are provided', () => {
    expect(filterByMediaText(items, { fileName: 'aula', description: '#jeangrey' })).toEqual([items[0]]);
    expect(filterByMediaText(items, { fileName: '02', description: '#jeangrey' })).toEqual([]);
  });
});
